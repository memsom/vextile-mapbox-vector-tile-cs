    ...
    public const cInt hiRange = 0x7FFF;
      ...
      //------------------------------------------------------------------------------
                ...
                private OutPt AddLocalMinPoly(TEdge e1, TEdge e2, IntPoint pt)
                {
                    ...
                    if (prevE != null && prevE.OutIdx >= 0)
                    {
                        ...
                        if ((xPrev == xE) && (e.WindDelta != 0) && (prevE.WindDelta != 0) &&
                            SlopesEqual(new(xPrev, pt.Y), prevE.Top, new(xE, pt.Y), e.Top, m_UseFullRange))
                        {
          ...
          SetZ(ref pt, e1, e2);
                                ...
                                if (dir == Direction.dLeftToRight)
                                {
                                    while (currMax != null && currMax.X < e.Curr.X)
                                    {
                                        if (horzEdge.OutIdx >= 0 && !IsOpen)
                                            AddOutPt(horzEdge, new(currMax.X, horzEdge.Bot.Y));
                                        currMax = currMax.Next;
                                    }
                                }
                                else
                    ...
                    if (IsSum)
                        for (int i = 0; i < pathCnt; i++)
                        {
                            ...
                            foreach (IntPoint ip in pattern)
                                p.Add(new(path[i].X + ip.X, path[i].Y + ip.Y));
                            result.Add(p);
                        }
                    else
                        for (int i = 0; i < pathCnt; i++)
                ...
                {
                    int pathCnt = path.Count;
                    Path outPath = new(pathCnt);
                    for (int i = 0; i < pathCnt; i++)
                    {
                        outPath.Add(new(path[i].X + delta.X, path[i].Y + delta.Y));
                    }
            ...
            public class ClipperOffset
            {
                ...
                public void AddPath(Path path, JoinType joinType, EndType endType)
                {
                    ...
                    if (endType != EndType.etClosedPolygon)
                        return;
                    if (m_lowest.X < 0)
                        m_lowest = new(m_polyNodes.ChildCount - 1, k);
                    else
                    {
                ...
                private void DoOffset(double delta)
                {
                    ...
                    for (int i = 0; i < m_polyNodes.ChildCount; i++)
                    {
                        ...
                        if (node.m_endtype == EndType.etClosedLine ||
                            ...
                            m_normals.Add(new(m_normals[len - 2]));
                        ...
                        else if (node.m_endtype == EndType.etClosedLine)
                        {
                            ...
                            for (int j = len - 1; j > 0; j--)
                                m_normals[j] = new(-m_normals[j - 1].X, -m_normals[j - 1].Y);
                            m_normals[0] = new(-n.X, -n.Y);
                            k = 0;
                            for (int j = len - 1; j >= 0; j--)
                                OffsetPoint(j, ref k, node.m_jointype);
                            m_destPolys.Add(m_destPoly);
                        ...
                        else
                        {
                            ...
                            {
                                int j = len - 1;
                                pt1 = new((cInt)Round(m_srcPoly[j].X + m_normals[j].X *
                            ...
                            else
                            {
                                ...
                                k = len - 2;
                                m_sinA = 0;
                                m_normals[j] = new(-m_normals[j].X, -m_normals[j].Y);
                            ...
                            m_normals[0] = new(-m_normals[1].X, -m_normals[1].Y);
                            ...
                            if (node.m_endtype == EndType.etOpenButt)
                            {
                                pt1 = new((cInt)Round(m_srcPoly[0].X - m_normals[0].X * delta),
                ...
                public void Execute(ref Paths solution, double delta)
                {
                    ...
                    else
                    {
                        ...
                        outer.Add(new(r.left - 10, r.bottom + 10));
                        outer.Add(new(r.right + 10, r.bottom + 10));
                        outer.Add(new(r.right + 10, r.top - 10));
                        outer.Add(new(r.left - 10, r.top - 10));
                ...
                void OffsetPoint(int j, ref int k, JoinType jointype)
                {
                    ...
                    if (Math.Abs(m_sinA * m_delta) < 1.0)
                    {
                        ...
                        {
                            m_destPoly.Add(new(Round(m_srcPoly[j].X + m_normals[k].X * m_delta),
                    ...
                    if (m_sinA * m_delta < 0)
                    {
                        ...
                        m_destPoly.Add(new(Round(m_srcPoly[j].X + m_normals[j].X * m_delta),
                ...
                {
                    double q = m_delta / r;
                    m_destPoly.Add(new(Round(m_srcPoly[j].X + (m_normals[k].X + m_normals[j].X) * q),
